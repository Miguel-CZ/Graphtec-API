# gl100/core/device.py
from gl100.core.commands import *
from gl100.utils.log import logger
from connection.usb_connection import USBConnection
from connection.wlan_connection import WLANConnection


class GL100Device:
    """Control y configuración de parámetros del GL100."""
    def __init__(self, connection:USBConnection | WLANConnection):
        self.connection = connection
        self.channels = {}

        try:
            #* Mandar el formato de EOL nada más iniciar conexión para evitar errores.
            self.connection.send(SET_CONN_NLCODE.format(code="CR_LF"))
        except Exception as e:
            logger.error(f"[GL100 Device] Error al configurar eol: {e}")

    # =========================================================
    # Medición
    # =========================================================
    def start_measurement(self):
        logger.debug("Iniciando medición...")
        self.connection.send(START_MEASUREMENT)

    def stop_measurement(self):
        logger.debug("Deteniendo medición...")
        self.connection.send(STOP_MEASUREMENT)

    def get_measurement_status(self):
        return self.connection.query(GET_STATUS)

    # =========================================================
    # Configuración de canales
    # =========================================================
    def get_channels(self)->dict:
        self.channels.clear()
        for channel in range(1,5):
            input_type = self.get_channel_input(channel)

            input_range = self.get_channel_range(channel)

            #Esto lo he movido a las funciones individuales. Es redundante en principio.
            #!Probar con el dispositivo y borrar si no es necesario.
            self.channels[channel] = {
                "input": input_type,
                "range": input_range
            }
        return self.channels
    
    def get_channel_input(self, channel):
        self.connection.send(GET_INPUT_TYPE.format(ch=channel))
        input_type=self._get_last_token(self.connection.receive_until().decode())

        # Modificamos el estado interno con la consulta más reciente
        self.channels[channel]["input"]=input_type

        return input_type
    
    def set_channel_input(self, channel, mode):
        command = SET_INPUT_TYPE.format(ch=channel, mode=mode)

        self.connection.send(command)

    def get_channel_range(self, channel):
        self.connection.send(GET_INPUT_RANGE.format(ch=channel))
        input_type=self._get_last_token(self.connection.receive_until().decode())

        # Modificamos el estado interno con la consulta más reciente
        self.channels[channel]["range"]=input_type

        return input_type

    def set_channel_range(self, channel, value):
        cmd = SET_INPUT_RANGE.format(ch=channel, value=value)
        self.connection.send(cmd)
    
    def _get_last_token(self,response: str | None)-> str:
            if response:
                response = response.replace(","," ").strip()
                parts = response.split()
                return parts[-1] if parts else ""
            else:
                return ""

    # =========================================================
    # Trigger
    # =========================================================
    def set_trigger_source(self, source):
        cmd = SET_TRIG_SOURCE.format(source=source)
        self.connection.send(cmd)

    def set_trigger_level(self, ch, value, edge="high"):
        cmd = SET_TRIG_LEVEL.format(ch=ch, value=value)
        self.connection.send(cmd)

    def get_trigger_status(self):
        return self.connection.query(":TRIG:COND?")

    # =========================================================
    # Alarma
    # =========================================================
    def set_alarm(self, ch, value, mode="high"):
        cmd = SET_ALARM_LEVEL.format(ch=ch, value=value)
        self.connection.send(cmd)

    def enable_alarm_output(self, enabled=True):
        state = "ON" if enabled else "OFF"
        cmd = ENABLE_ALARM_OUT.format(state=state)
        self.connection.send(cmd)

    def get_alarm_status(self):
        return self.connection.query(":ALAR:FUNC?")

    # =========================================================
    # Archivos
    # =========================================================
    def list_files(self, path="/"):
        return self.connection.query(LIST_FILES)

    def save_settings(self, filename):
        cmd = SAVE_SETTINGS.format(filename=filename)
        self.connection.send(cmd)

    def load_settings(self, filename):
        cmd = LOAD_SETTINGS.format(filename=filename)
        self.connection.send(cmd)

    def delete_file(self, filename):
        cmd = DELETE_FILE.format(filename=filename)
        self.connection.send(cmd)

    def get_free_space(self):
        return self.connection.query(GET_FREE_SPACE)

    # =========================================================
    # Sistema
    # =========================================================
    def get_datetime(self):
        return self.connection.query(GET_DATETIME)

    def set_datetime(self, dt):
        cmd = SET_DATETIME.format(datetime=dt)
        self.connection.send(cmd)

    def get_status_flags(self):
        return self.connection.query(GET_STATUS)

    def clear_status(self):
        self.connection.send(CLEAR_STATUS)

    def get_firmware_version(self):
        return self.connection.query(GET_IDN)

    def get_model_info(self):
        return self.connection.query(GET_IDN)
